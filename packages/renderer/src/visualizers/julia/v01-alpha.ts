...
const juliaMetadata: VisualizerMetadata = {
  ...
  params: [
    {
      key: 'seedPath',
      label: 'Seed path (0 artistic · 1 cardioid · 2 bulb · 3 dust)',
      min: 0,
      max: 3,
      step: 1,
      initial: 1,
      category: 'appearance',
      description:
        'Cardioid and period-two paths have attracting cycles; dust seeds escape the Mandelbrot set',
    },
    // Appearance
    {
      key: 'orbitSpeed',
    ...
    // Audio mapping strengths
    {
      ...
      description: 'How strongly bass pushes the c-parameter orbit',
    },
    {
      key: 'midToZoom',
      label: 'Mid \u2192 Zoom',
      ...
      initial: 1.0,
      category: 'audio-mapping',
      description: 'How strongly mids drive zoom breathing',
    },
    {
      key: 'rmsToBrightness',
...
export class JuliaVisualizer implements Visualizer {
  ...
  constructor(private bus: MessageBus) {
    ...
    this.smoothers.zoom.reset(0.38);
  ...
  tick(deltaSeconds = 1 / 60): void {
    ...
    this.transitionTime += this.deltaSeconds;

    // Orbit the c-parameter around the current base point
    this.orbitPhase += this.orbitSpeed * this.deltaSeconds;
    ...
    if (this.latestFeatures) {
      ...
      const audioRadius =
        this.orbitRadius * (0.5 + f.bass * 1.5 * am.bassToOrbit);
      ...
      this.smoothers.cx.update(cx, this.deltaSeconds);
      this.smoothers.cy.update(cy, this.deltaSeconds);
      if (!this.viewOverrides.zoom) {
        this.smoothers.zoom.update(
          0.38 + f.mid * 0.04 * am.midToZoom,
          this.deltaSeconds,
        );
      }
      ...
      this.smoothers.colorWarmth.update(f.spectralCentroid, this.deltaSeconds);
      ...
      this.smoothers.cx.update(cx, this.deltaSeconds);
      this.smoothers.cy.update(cy, this.deltaSeconds);
      this.smoothers.beatPulse.update(0, this.deltaSeconds);
    }

    if (!this.viewOverrides.seed && this.seedPath > 0) {
      const phase = this.orbitPhase * 0.35;
      const bass = this.latestFeatures?.bass ?? 0;
      const radius = Math.min(
        0.85,
        0.55 +
          this.orbitRadius * 0.5 +
          bass * this.audioMapStrengths.bassToOrbit * 0.08,
      );
      ...
      if (this.seedPath === 1) {
        ...
        cx = x / 2 - (x * x - y * y) / 4;
        cy = y / 2 - (x * y) / 2;
      } else if (this.seedPath === 2) {
        cx = -1 + 0.15 * Math.cos(phase);
        cy = 0.15 * Math.sin(phase);
      } else {
        cx = 0.4 + 0.025 * Math.cos(phase);
        cy = 0.025 * Math.sin(phase);
      }
      this.smoothers.cx.reset(cx);
      this.smoothers.cy.reset(cy);
    }
    if (this.material) {
      this.material.uniforms.u_c.value.set(
        this.smoothers.cx.value,
        this.smoothers.cy.value,
      );
      this.material.uniforms.u_zoom.value = this.smoothers.zoom.value;
      this.material.uniforms.u_iterations.value = Math.round(
        Math.min(this.smoothers.iterations.value, 800),
      );
      this.material.uniforms.u_colorSpeed.value =
        this.smoothers.colorSpeed.value;
      this.material.uniforms.u_brightness.value =
        this.smoothers.brightness.value + this.smoothers.beatPulse.value * 0.25;
  ...
  setUserParam(key: string, value: number): void {
    switch (key) {
      case 'seedPath':
        this.seedPath = Math.round(value);
        this.viewOverrides.seed = false;
        break;
      case 'orbitSpeed':
...
const FRAGMENT_SHADER = /* glsl */ `
  ...
  void main() {
    ...
    int maxIter = int(clamp(u_iterations * u_workBudget, 32.0, 500.0));
    int i;
    for (i = 0; i < 800; i++) {
      if (i >= maxIter) break;
