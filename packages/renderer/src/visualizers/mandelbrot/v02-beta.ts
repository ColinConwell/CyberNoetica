...
const ZOOM_TARGETS = [
  { cx: -1.76, cy: -0.03, name: 'Main Ship' },
  { cx: -1.749, cy: -0.01, name: 'Mast' },
  { cx: -1.626, cy: 0.04, name: 'Antenna' },
  { cx: -1.94, cy: 0.0, name: 'Left Prow' },
  { cx: -1.755, cy: -0.0285, name: 'Hull Mini' },
  { cx: 0.23, cy: -1.1, name: 'Lower Satellite' },
];
...
const burningShipMetadata: VisualizerMetadata = {
  ...
  params: [
    ...
    },
    {
      ...
      initial: 120,
      category: 'appearance',
      description: 'Escape-time iteration ceiling',
    },
    {
      key: 'brightness',
      label: 'Brightness',
    ...
    },
    {
      ...
      description: 'Bass drives zoom rate',
    },
    {
      key: 'midToIters',
      label: 'Mid \u2192 Detail',
      ...
      initial: 1.0,
      category: 'audio-mapping',
      description: 'Mids push iteration count',
    },
    {
      key: 'highToShimmer',
      label: 'High \u2192 Shimmer',
      ...
      initial: 1.0,
      category: 'audio-mapping',
      description: 'Highs brighten fractal edges',
    },
    {
      key: 'rmsToBrightness',
      ...
      initial: 1.0,
      category: 'audio-mapping',
      description: 'Volume sets overall brightness',
    },
    {
      key: 'beatToJolt',
      label: 'Beat \u2192 Jolt',
      ...
      initial: 1.0,
      category: 'audio-mapping',
      description: 'Beats spin the rotation',
    },
  ],
...
export class BurningShipVisualizer implements Visualizer {
  ...
  tick(deltaSeconds = 1 / 60): void {
    ...
    const bassBoost =
      this.smoothers.bass.value * 0.8 * this.userParams.bassToZoom;
    ...
    if (!this.viewOverrides.zoom) {
      if (this.zoomDirection === 'in') {
        this.zoomLevel *= Math.exp(
          Math.log1p(zoomRate) * this.deltaSeconds * 60,
        );
        if (this.zoomLevel >= this.maxZoom) this.zoomDirection = 'out';
      } else {
        this.zoomLevel *= Math.exp(
          Math.log1p(-zoomRate * 1.5) * this.deltaSeconds * 60,
        );
        if (this.zoomLevel <= this.minZoom) {
          ...
          if (!this.viewOverrides.centerReal) this.cx = this.target.cx;
          if (!this.viewOverrides.centerImaginary) this.cy = this.target.cy;
        }
      }
    }
    ...
    if (!this.viewOverrides.rotation) {
      this.rotation +=
        ...
        (1.0 +
          this.smoothers.beatPulse.value * 0.5 * this.userParams.beatToJolt) *
        this.deltaSeconds *
        60;
    ...
    const iters =
      this.userParams.maxIterations *
      (1.0 + this.smoothers.mid.value * 0.3 * this.userParams.midToIters);
    ...
    if (this.material) {
      ...
      u.u_brightness.value =
        ...
        (0.7 + this.smoothers.rms.value * this.userParams.rmsToBrightness);
      u.u_hueShift.value = this.userParams.hueShift;
      u.u_rms.value = this.smoothers.rms.value;
      u.u_high.value =
        this.smoothers.high.value * this.userParams.highToShimmer;
  ...
  setViewState(partial: Record<string, number>): void {
    ...
    if ('zoom' in partial) {
      this.zoomLevel = Math.max(0.5, Math.min(1000, partial.zoom));
      this.viewOverrides.zoom = true;
    }
    if ('rotation' in partial) {
      this.rotation =
        ((partial.rotation % (Math.PI * 2)) + Math.PI * 2) % (Math.PI * 2);
...
const FRAGMENT_SHADER = /* glsl */ `
  ...
  void main() {
    ...
    // The viewport y-axis is reflected above; the selected center is unchanged.

    vec2 z = vec2(0.0);
    float iterations = 0.0;
    int maxIter = int(clamp(u_maxIterations * u_workBudget, 32.0, 300.0));
    float escape = 0.0;
    ...
    float t = clamp(iterations / float(maxIter), 0.0, 1.0);
