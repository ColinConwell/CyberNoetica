...
import { NewtonVisualizer } from '../visualizers/newton/v01-alpha.js';
import { MessageBus } from '@cybernoetica/core';
import type { AudioFeatures } from '@cybernoetica/core';

describe('NewtonVisualizer', () => {
  it('can be constructed', () => {
    const bus = new MessageBus();
    const viz = new NewtonVisualizer(bus);
  ...
  it('responds to audio features', () => {
    ...
    viz.tick();
    expect(viz.metadata.type).toBe('newton');
  });

  it('has correct metadata', () => {
    const bus = new MessageBus();
    const viz = new NewtonVisualizer(bus);
    expect(viz.metadata.label).toBe('Newton Fractal');
  ...
  it('setUserParam updates exponent', () => {
    const bus = new MessageBus();
    const viz = new NewtonVisualizer(bus);
    viz.setUserParam('exponent', 6);
    viz.tick();
    expect(viz.metadata.type).toBe('newton');
  });
