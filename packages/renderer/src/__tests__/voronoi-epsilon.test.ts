...
import { VoronoiEpsilonVisualizer } from '../visualizers/voronoi/v05-epsilon.js';
import { MessageBus } from '@cybernoetica/core';
import type { AudioFeatures } from '@cybernoetica/core';

describe('VoronoiEpsilonVisualizer', () => {
  it('can be constructed', () => {
    const bus = new MessageBus();
    const viz = new VoronoiEpsilonVisualizer(bus);
  ...
  it('has correct metadata type and label', () => {
    const bus = new MessageBus();
    const viz = new VoronoiEpsilonVisualizer(bus);
    expect(viz.metadata.type).toBe('voronoi-epsilon');
    expect(viz.metadata.label).toBe('Voronoi (ε)');
  ...
  it('setUserParam updates params including sphereRadius', () => {
    const bus = new MessageBus();
    const viz = new VoronoiEpsilonVisualizer(bus);
    viz.setUserParam('seedCount', 32);
    viz.setUserParam('sphereRadius', 1.5);
    viz.setUserParam('rmsToRadius', 1.2);
    viz.tick();
    expect(viz.metadata.params.find(p => p.key === 'sphereRadius')).toBeDefined();
    expect(viz.metadata.params.find(p => p.key === 'rmsToRadius')).toBeDefined();
  });
