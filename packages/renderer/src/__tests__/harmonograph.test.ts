...
import { HarmonographVisualizer } from '../visualizers/harmonograph/v01-alpha.js';
import { MessageBus } from '@cybernoetica/core';
import type { AudioFeatures } from '@cybernoetica/core';

describe('HarmonographVisualizer', () => {
  it('can be constructed', () => {
    const bus = new MessageBus();
    const viz = new HarmonographVisualizer(bus);
  ...
  it('responds to audio features', () => {
    ...
    viz.tick();
    expect(viz.metadata.type).toBe('harmonograph');
  });

  it('has correct metadata', () => {
    const bus = new MessageBus();
    const viz = new HarmonographVisualizer(bus);
    expect(viz.metadata.label).toBe('Harmonograph');
