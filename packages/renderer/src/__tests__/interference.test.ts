...
import { InterferenceVisualizer } from '../visualizers/interference/v01-alpha.js';
import { MessageBus } from '@cybernoetica/core';
import type { AudioFeatures } from '@cybernoetica/core';

describe('InterferenceVisualizer', () => {
  it('can be constructed', () => {
    const bus = new MessageBus();
    const viz = new InterferenceVisualizer(bus);
  ...
  it('responds to audio features', () => {
    ...
    viz.tick();
    expect(viz.metadata.type).toBe('interference');
  });

  it('has correct metadata', () => {
    const bus = new MessageBus();
    const viz = new InterferenceVisualizer(bus);
    expect(viz.metadata.label).toBe('Interference');
