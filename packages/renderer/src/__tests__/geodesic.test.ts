...
import { GeodesicVisualizer } from '../visualizers/geodesic/v01-alpha.js';
import { MessageBus } from '@cybernoetica/core';
import type { AudioFeatures } from '@cybernoetica/core';

describe('GeodesicVisualizer', () => {
  it('can be constructed', () => {
    const bus = new MessageBus();
    const viz = new GeodesicVisualizer(bus);
  ...
  it('responds to audio features', () => {
    ...
    viz.tick();
    expect(viz.metadata.type).toBe('geodesic');
  });

  it('has correct metadata', () => {
    const bus = new MessageBus();
    const viz = new GeodesicVisualizer(bus);
    expect(viz.metadata.label).toBe('Geodesic');
