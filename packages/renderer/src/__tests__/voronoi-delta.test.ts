...
import { VoronoiDeltaVisualizer } from '../visualizers/voronoi/v04-delta.js';
import { MessageBus } from '@cybernoetica/core';
import type { AudioFeatures } from '@cybernoetica/core';

describe('VoronoiDeltaVisualizer', () => {
  it('can be constructed', () => {
    const bus = new MessageBus();
    const viz = new VoronoiDeltaVisualizer(bus);
  ...
  it('has correct metadata type and label', () => {
    const bus = new MessageBus();
    const viz = new VoronoiDeltaVisualizer(bus);
    expect(viz.metadata.type).toBe('voronoi-delta');
    expect(viz.metadata.label).toBe('Voronoi (δ)');
  ...
  it('setUserParam updates params including radiusContrast', () => {
    const bus = new MessageBus();
    const viz = new VoronoiDeltaVisualizer(bus);
    viz.setUserParam('seedCount', 32);
    viz.setUserParam('radiusContrast', 1.5);
    viz.tick();
    expect(viz.metadata.params.find(p => p.key === 'radiusContrast')).toBeDefined();
  });
