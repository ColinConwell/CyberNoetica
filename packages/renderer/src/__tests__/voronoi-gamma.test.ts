...
import { VoronoiGammaVisualizer } from '../visualizers/voronoi/v03-gamma.js';
import { MessageBus } from '@cybernoetica/core';
import type { AudioFeatures } from '@cybernoetica/core';

describe('VoronoiGammaVisualizer', () => {
  it('can be constructed', () => {
    const bus = new MessageBus();
    const viz = new VoronoiGammaVisualizer(bus);
  ...
  it('has correct metadata type and label', () => {
    const bus = new MessageBus();
    const viz = new VoronoiGammaVisualizer(bus);
    expect(viz.metadata.type).toBe('voronoi-gamma');
    expect(viz.metadata.label).toBe('Voronoi (γ)');
