...
import { LorenzBetaVisualizer } from '../visualizers/lorenz/v02-beta.js';
import { MessageBus } from '@cybernoetica/core';
import type { AudioFeatures } from '@cybernoetica/core';

describe('LorenzBetaVisualizer', () => {
  it('can be constructed', () => {
    const bus = new MessageBus();
    const viz = new LorenzBetaVisualizer(bus);
  ...
  it('has correct metadata type and label', () => {
    const bus = new MessageBus();
    const viz = new LorenzBetaVisualizer(bus);
    expect(viz.metadata.type).toBe('lorenz-beta');
    expect(viz.metadata.label).toBe('Rössler');
  ...
  it('ticks without audio (idle mode)', () => {
    ...
    expect(viz.getViewState().distance).toBe(10);
  });
  ...
  it('getViewState returns valid initial camera coordinates', () => {
    ...
    expect(vs.orbitAngle).toBe(0.5);
    expect(vs.elevation).toBe(0.4);
    expect(vs.distance).toBe(10);
  });
