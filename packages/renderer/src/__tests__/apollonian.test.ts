...
import { ApollonianVisualizer } from '../visualizers/apollonian/v01-alpha.js';
import { MessageBus } from '@cybernoetica/core';
import type { AudioFeatures } from '@cybernoetica/core';

describe('ApollonianVisualizer', () => {
  it('can be constructed', () => {
    const bus = new MessageBus();
    const viz = new ApollonianVisualizer(bus);
  ...
  it('responds to audio features', () => {
    ...
    viz.tick();
    expect(viz.metadata.type).toBe('apollonian');
  });

  it('has correct metadata', () => {
    const bus = new MessageBus();
    const viz = new ApollonianVisualizer(bus);
    expect(viz.metadata.label).toBe('Apollonian Gasket');
  ...
  it('setUserParam updates iterations', () => {
    const bus = new MessageBus();
    const viz = new ApollonianVisualizer(bus);
    viz.setUserParam('iterations', 12);
    viz.tick();
    expect(viz.metadata.type).toBe('apollonian');
  });
