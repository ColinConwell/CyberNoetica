...
import { PlasmaVisualizer } from '../visualizers/plasma/v01-alpha.js';
import { MessageBus } from '@cybernoetica/core';
import type { AudioFeatures } from '@cybernoetica/core';

describe('PlasmaVisualizer', () => {
  it('can be constructed', () => {
    const bus = new MessageBus();
    const viz = new PlasmaVisualizer(bus);
  ...
  it('responds to audio features', () => {
    ...
    viz.tick();
    expect(viz.metadata.type).toBe('plasma');
  });

  it('has correct metadata', () => {
    const bus = new MessageBus();
    const viz = new PlasmaVisualizer(bus);
    expect(viz.metadata.label).toBe('Plasma');
