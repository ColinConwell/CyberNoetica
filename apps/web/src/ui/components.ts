...
export function toggleSwitch(opts: ToggleSwitchOpts = {}): ToggleSwitchHandle {
  ...
  container.setAttribute('aria-label', opts.label ?? 'Toggle');
  ...
  function update() {
    ...
    thumb.style.background = checked
      ? 'rgba(255, 255, 255, 0.95)'
      : 'rgba(255, 255, 255, 0.5)';
    thumb.style.boxShadow = checked
      ? '0 0 6px rgba(140, 160, 255, 0.5)'
      : 'none';
  }
...
export function infoBlock(fields: InfoBlockField[]): HTMLElement {
  ...
  for (let i = 0; i < fields.length; i++) {
    ...
    const value = el('span', {
      fontSize: '10px',
      color: field.color || TEXT_PRIMARY,
...
export function paramSlider(opts: ParamSliderOpts): HTMLElement {
  ...
  const slider = el(
    'input',
    { width: '90px', accentColor: ACCENT },
    {
      type: 'range',
      min: String(opts.min),
      max: String(opts.max),
      step: String(opts.step),
      value: String(opts.initial),
    },
  );
  slider.setAttribute('aria-label', opts.label);
  if (opts.description)
    slider.setAttribute('aria-description', opts.description);
  const num = el('span', {
    fontSize: '10px',
    color: TEXT_DIM,
    minWidth: '36px',
...
export function sectionDivider(): HTMLElement {
  return el('div', {
    height: '1px',
    background:
      'linear-gradient(90deg, transparent, rgba(140, 160, 255, 0.2), transparent)',
    margin: '14px 0',
  });
}
